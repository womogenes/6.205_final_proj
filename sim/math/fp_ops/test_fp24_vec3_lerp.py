...
async def test_module(dut):
    ...
    t_fp24 = list(map(make_fp24, t))
    one_min_t_fp24 = list(map(make_fp24, 1 - t))
    ...
    for i in range(N_SAMPLES):
        ...
        dut.t.value = t_fp24[i]
        dut.one_min_t.value = one_min_t_fp24[i]

        # await ClockCycles(dut.clk, 10)
        await RisingEdge(dut.clk)
        dut_ans.append(convert_fp24_vec3(dut.lerped.value))
...
def runner():
    ...
    sources = [
        ...
        proj_path / "hdl" / "math" / "fp24_vec3_lerp.sv",
    ]
